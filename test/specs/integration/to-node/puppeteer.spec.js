...
describe("features that are in Prerenderer's Puppeteer", () => {
  it('should follow resource redirects.', async () => {
    const { response } = await createDirectHttpGetRequest('redirect-resource/index.html', {}, true);
    const $ = cheerio.load(response.body);
    assert.equal($('#app').length, 1);
  });

  it('should use resources whitelist.', async () => {
    const { response } = await createDirectHttpGetRequest('whitelist/index.html', {}, true);
    const $ = cheerio.load(response.body);
    assert.equal($('#app').length, 1);
  });

  it('should use resources blacklist.', async () => {
    const { response } = await createDirectHttpGetRequest('blacklist/index.html', {}, true);
    const $ = cheerio.load(response.body);
  ...
  it('should see response with status 400 for aborted request, empty response.', async () => {
    const { response } = await createDirectHttpGetRequest('status/304.html', {}, true);
    assert.isEmpty(response.body);
    assert.equal(response.statusCode, 400);
  });

  it('should see response with status 400.', async () => {
    const { response } = await createDirectHttpGetRequest('status/400.html', {}, true);
    const $ = cheerio.load(response.body);
    assert.equal(response.statusCode, 400);
    assert.equal($('body').length, 1);
    assert.equal($('#app').length, 0);
